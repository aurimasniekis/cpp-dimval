# Convenience wrapper around CMake/CTest workflows for dimval.
...
help:
	...
	@echo "  make example           Run the dimval_basic_unit example"
	@echo "  make examples          Build and run every dimval_* example (fails on any non-zero exit)"
	...
	@echo "  make no-json           Configure+build+test in build-no-json/ with DIMVAL_WITH_NLOHMANN_JSON=OFF (parcel also disabled, since it depends on json)"
	@echo ""
...
.PHONY: example
example: build
	$(BUILD_DIR)/examples/dimval_basic_unit

.PHONY: examples
examples: build
	@set -e; \
	failures=""; \
	for ex in $(BUILD_DIR)/examples/dimval_*; do \
		if [ -x "$$ex" ] && [ ! -d "$$ex" ]; then \
			name=$$(basename "$$ex"); \
			echo "=== $$name ==="; \
			if ! "$$ex" >/dev/null; then \
...
.PHONY: sanitize
sanitize:
	$(CMAKE) -S . -B build-san $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DDIMVAL_ENABLE_SANITIZERS=ON
	$(CMAKE) --build build-san -j $(JOBS)
	$(CTEST) --test-dir build-san --output-on-failure

.PHONY: tidy
tidy:
	$(CMAKE) -S . -B build-tidy $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DDIMVAL_ENABLE_CLANG_TIDY=ON
...
.PHONY: no-json
no-json:
	$(CMAKE) -S . -B build-no-json $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DDIMVAL_WITH_NLOHMANN_JSON=OFF -DDIMVAL_WITH_PARCEL=OFF
...
coverage:
	$(CMAKE) -S . -B build-coverage $(CMAKE_GEN_FLAG) \
		-DCMAKE_BUILD_TYPE=Debug -DDIMVAL_ENABLE_COVERAGE=ON
	$(CMAKE) --build build-coverage -j $(JOBS)
	rm -f build-coverage/*.profraw build-coverage/dimval.profdata
	LLVM_PROFILE_FILE="$(CURDIR)/build-coverage/dimval-%p.profraw" \
		$(CTEST) --test-dir build-coverage --output-on-failure
	$(LLVM_PROFDATA) merge -sparse build-coverage/*.profraw \
		-o build-coverage/dimval.profdata
	$(LLVM_COV) report build-coverage/tests/dimval_tests \
		-instr-profile=build-coverage/dimval.profdata \
		-ignore-filename-regex='(_deps|tests)/'
	$(LLVM_COV) show build-coverage/tests/dimval_tests \
		-instr-profile=build-coverage/dimval.profdata \
		-ignore-filename-regex='(_deps|tests)/' \
...
.PHONY: docs
docs:
	$(CMAKE) -S . -B build-docs $(CMAKE_GEN_FLAG) -DDIMVAL_BUILD_DOCS=ON
	$(CMAKE) --build build-docs --target dimval_docs -j $(JOBS)
